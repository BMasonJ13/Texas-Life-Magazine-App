...
const Tastes = ({ isAdmin }) => {
    ...
    const getData = async () => {
        setData(await getCards("Articles-tastes"))
        setSponsorData(await getAds("sponsor-tastes"))
        setBannerData(await getAds("sponsor-long-tastes"))
    }
    ...
    return (
        <>
            {isAdmin && <AddContentCard cardStyle={styles.card} action="Add Article" path="/AddArticle/:Tastes" />}
